...
export const SANDIFY_FILES_ROOT_URL =
  localStorage?.getItem("sandify:files-host") ?? 'https://sandify-files.besson.co'
...
const mapStateToProps = (state, ownProps) => {
  ...
  return {
    ...
    layer: main.layers.current,
    layersById: main.layers.byId,
...
class Downloader extends Component {
  ...
  share() {
    window.open(`${SANDIFY_FILES_ROOT_URL}/new`, "_blank")
    const messageListener = (event) => {
      if (
        event.origin === SANDIFY_FILES_ROOT_URL &&
        event?.data?.type === "readyForDrawing"
      ) {
        event.source.postMessage(
          {
            type: "drawingData",
            layers: Object.values(this.props.layersById),
